				...
				<div class="card col-sm-3 p-0 mr-2 mb-2" style="width: 8rem;">
					<div class="card-body">
						...
						<div class="text-center">
							<a href="<?php echo (get_post_status($user_resume->ID) == 'draft') ? (get_home_url() . '/?post_type=resume&p=' . $user_resume->ID . '&preview=true') : ( get_post_permalink($user_resume->ID)) ?>" target="_blank" class="card-link">                                
								Preview
							</a>
							<a href="<?php echo esc_url($edit_url) ?>" class="card-link">Edit</a>
							<a onclick="return confirm('Are you Sure you want to Delete this post?')" href="<?php echo get_delete_post_link($user_resume->ID) ?>" class="card-link">Delete</a>
							<a href="<?php //  echo wp_publish_post($user_resume->ID)  ?>" class="card-link">
								Publish
							</a>
						</div>
					</div>
